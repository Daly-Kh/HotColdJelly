...
def collect_candidates(items_map):
    """
    Walk all hot directories and classify each item as archive or keep.
        ...
        if is_shows:
            ...
            for root, _, files in os.walk(hot_dir):
                ...
                for fname in files:
                    ...
                    # Get item_id for Jellyfin metadata update
                    item_id = info.get("id") if info else None

                    print(f"\nArchiving: {fname} ({size_gb:.1f}GB)")
                    success, fail_reason = move_to_cold(
                        src, hot_dir,
                        item_id=item_id,
                        stats=stats
                    )
