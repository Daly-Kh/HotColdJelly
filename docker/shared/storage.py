...
def oldest_file_age_days(folder):
    """Age in days of oldest video file in folder tree."""
    oldest = None
...
def hot_to_cold_path(src, hot_dir):
    """Hot path → cold WebDAV mount path for symlink creation."""
...
def move_to_cold(src, hot_dir, item_id=None, stats=None):
    ...
    Streams directly to remote — zero local disk usage.

    Steps:
    1. Validate source
    2. Check if already on remote
    ...
    if item_id:
        try:
            ...
            logging.warning(f"Could not mark as cold for {fname}: {e}")
